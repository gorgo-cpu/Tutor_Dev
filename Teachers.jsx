...
const toCalendarEvent = (lesson) => ({
  id: lesson.id,
  title: lesson.title ? `${lesson.title} - Student ${shortId(lesson.student_id)}` : `Meeting - Student ${shortId(lesson.student_id)}`,
...
export default function Teachers({ onBack }) {
  ...
  const upcoming = useMemo(() => {
    const now = new Date()
    return [...events].filter((e) => e.start >= now).sort((a, b) => a.start - b.start).slice(0, 5)
  }, [events])
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex justify-center">
            <AuthForm initialMode="login" initialRole="teacher" />
          </div>
        </div>
      </div>
    )
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-gray-900">Teacher Dashboard</h1>
              <p className="text-sm text-gray-600">Your weekly schedule and student meetings.</p>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-4">
            <UpcomingList title="Upcoming meetings" events={upcoming} />
            {selectedEvent && (
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    {selectedEvent.student_id && (
                      <div className="text-xs text-gray-500 mt-1">Student ID: {selectedEvent.student_id}</div>
                    )}
